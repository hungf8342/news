...
topic_contain<-function(list,dataset) {
  ...
  for (i in length(list)) {
    ...
    new<-grepl(paste(c(list[i],paste(list[i],"s",sep=""),
          substr(list[i],1,nchar(as.character(list[i]))-1)),collapse="|"),dataset[,1])
    }
    else {
    new<-grepl(paste(c(list[i],paste(list[i],"s",sep="")),collapse="|"),dataset[,1])
    }
    p<-(p & new)
  }
  sum(p)/10
...
I'm planning on using neural nets (likely via TensorFlow)  to attempt to classify keyword patterns into conservative vs. liberal, extreme vs. moderate, etc. 